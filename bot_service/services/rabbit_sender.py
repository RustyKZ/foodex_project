...
RABBITMQ_URL = (
    f"amqp://{RABBIT_USERNAME}:{RABBIT_PASSWORD}@{RABBIT_HOST}/"
)
...
async def broadcast_message_async(message: dict) -> dict:
    ...
    async def _send():
        try:
            ...
            serialized = json.dumps(
                {"error": "Invalid message in broadcast"}
            )
        ...
        try:
            ...
            return {
                "status": False,
                "error_message": f"RabbitMQ publish error: {str(e)}"
            }
    ...
    try:
        ...
        return {
            "status": False,
            "error_message": (
                f"RabbitMQ broadcast timeout after {timeout} ms"
            )
        }
    

async def direct_task_async(receiver_service_name: str, message: dict) -> dict:

    routing_key = f"{receiver_service_name}.task"
