...
def put_critical_error_into_db(function_name, error_short, error_text, context):
    try:
        if not PUT_ERROR_INTO_DATABASE:
            logger.error(
                f"put_critical_error_into_db - NOT SAVED - "
                f"{function_name} - {error_short} - {error_text} - {context}"
            )
            return
        ...
        with sync_session() as session:
            ...
            logger.error(
                f"put_critical_error_into_db - SAVED - "
                f"{function_name} - {error_short} - {error_text} - {context}"
            )
        ...
        logger.error(
            f"put_critical_error_into_db - EXCEPTION - NOT SAVED - "
            f"{function_name} - {error_short} - {error_text} - {context}"
        )